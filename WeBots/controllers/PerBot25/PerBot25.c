...
#define WHEEL_RADIUS     0.025   // meters
#define AXLE_LENGTH      0.09     // meters
  ...
  while (wb_robot_step(get_time_step()) != -1) {
    if (LAP == 1) {
      ...
      if (red>0) {
        ...
        int delay_steps = (int)(1000 / get_time_step());  // 2000 ms = 2 seconds
        if(Finish){
        printf("Lap COUNT=%d\n", Lapcount);
        Lapcount++;
          for (int i = 0; i < delay_steps; i++) {
            ...
            break;  // Exit if simulation is terminated
            }
          }
          }
      } else if (RB1>RW1) {
      ...
      else if (Alightcount > 4) {
        if ((LW4 + LW3 + LW2 + LW1 ) > 800) {
        // printf("ALIGN\n");
        vl_cmd = -TURN_SPEED3;  vr_cmd=TURN_SPEED3;
        }
        Alightcount = 0;
